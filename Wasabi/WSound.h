...
public:
	...
	WError					LoadFromMemory(uint buffer, void* data, size_t dataSize, ALenum format, uint frequency, bool bSaveData = false);
	void					Play();
	void					Loop();
	void					Pause();
	void					Reset();
	...
	WError					LoadFromWS(basic_filebuf<char>* buff, uint pos, bool bSaveData = false);
